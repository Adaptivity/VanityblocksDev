...
public class Storageprops
{
    public static void initProps (File configBase)
    {
        ...
        enablestorageblocks = config.get(enabled, "Enable Storageblocks?", true).getBoolean(true);
        enablerupees = config.get(enabled, "Enabled the Rupees?", true).getBoolean(true);
        enableclaymugstuff = config.get(enabled, "Enable the Mug stuff(like hot chocolate)", true).getBoolean(true);
        furnacemelts = config.get(enabled, "Allow furnace melting of stuff like iron doors?", true).getBoolean(true);
        versioncheck = config.get(enabled, "Turn this to false to turn off the version check", true).getBoolean(true);
        enableredstonelamps = config.get(enabled, "Enable the redstone lamp colors?", true).getBoolean(true);
        enabletrapdoors = config.get(enabled, "Enable hidden trap doors", true).getBoolean(true);
        enablecurtains = config.get(enabled, "Enable the curtains?", true).getBoolean(true);
        enablerandomrecipes = config.get(enabled, "Enable random recipes load?", true).getBoolean(true);
        enablecompressblocks = config.get(enabled, "Enable the Common Compressed Blocks?", true).getBoolean(true);
        enablepillars = config.get(enabled, "Enable the Pillars?", true).getBoolean(true);
        enablepressureplate = config.get(enabled,"Enable the Pressure Plates?", true).getBoolean(true);
        // enablefences = config.get(enabled, "Enable the fences?", true)
        // .getBoolean(true);

        /* String blocks = "Block id's";
        trapdoorconfigs = config.getBlock(blocks, "Vanityblocks trapdoors id", 3049).getInt(3049);
        storageblockconfig = config.getBlock(blocks, "Vannila Storage Blocks ID", 3050).getInt(3050);
        storageblockmodconfig = config.getBlock(blocks, "Modded Storage Blocks ID", 3051).getInt(3051);
        forestryblockconfig = config.getBlock(blocks, "Forestry Storage Blocks ID", 3053).getInt(3053);
        vanitydesignconfig = config.getBlock(blocks, "Vanity design blocks ID", 3055).getInt(3055);
        redstonelampdimconfig = config.getBlock(blocks, "Redstone Lamp Dim Id", 3056).getInt(3056);
        redstonelamplitconfig = config.getBlock(blocks, "Redstone lamp Lit id", 3057).getInt(3057);
        vanitydesignworldconfig = config.getBlock(blocks, "Vanity Blocks World Generation block ID", 3058).getInt(3058);
        vanityentityconfig = config.getBlock(blocks, "Vanity Blocks Tile Entity(such as melting core) ID", 3060).getInt(3058);
        vanitydesignworldslabconfig = config.getBlock(blocks, "Vanity Blocks World Gen slabs ID", 3061).getInt(3061);
        vanitydesignworldWallconfig = config.getBlock(blocks, "Vanity Blocks World Gen walls ID", 3062).getInt(3062);
        curtainblockconfig = config.get(blocks, "Vanity Curtain Id", 3063).getInt(3063);
        randomblocksconfig = config.getBlock(blocks, "Vanityblocks random blocks id", 3064).getInt(3064);
		...
		*/
        ...
        enablegeneralitems = config.get(generalitems, "Enable the general items?", true).getBoolean(true);
        enablecoalstorageitem = config.get(generalitems, "Enable the coal storage item?", true).getBoolean(true);
        coalstorageitem = config.get(generalitems, "Coal Storage Item Id", 19009).getInt(19009);
        enableflintitem = config.get(generalitems, "Enable Flint Item?", true).getBoolean(true);
        flintitem = config.get(generalitems, "Flint Storage Item Id", 19010).getInt(19010);
        ...
        mugwaterhot = config.get(generalfooditems, "Mug with hot water Id", 19023).getInt(19023);
        mugwaterhotchoco = config.get(generalfooditems, "Hot Chocolate with water Id", 19024).getInt(19024);
        mugwaterhotchocosugar = config.get(generalfooditems, "Sweetend Hot Chocolate with water Id", 19025).getInt(19025);
        mugmilk = config.get(generalfooditems, "Mug with milk Id", 19026).getInt(19026);
		...
		*/
        ...
        enablemeltingcore = config.get(category1, "Enable Melting Core crafting?/Enable it", true).getBoolean(true);
        ...
        nametag = config.get(category3, "Allow Nametags to be crafted(sign + string)", true).getBoolean(true);
        ironhorsearmor = config.get(category3, "Allow Iron Horse armor to be crafted(upside down iron chestplate)", true).getBoolean(true);
        ...
        marblestairs = config.get(category8, "Enabled/allow White marble stairs?(note marble needs to be enabled)", true).getBoolean(true);
        blackmarblestairs = config.get(category8, "Enabled/Allow Black marble stairs?(note black marble needs to be enabled)", true).getBoolean(true);
        ...
        enablerandomblocks = config.get(category9, "Enable the random blocks?", true).getBoolean(true);
        enableclaybrick = config.get(category9, "Enable clay brick crafting", true).getBoolean(true);
        enableoldlapisblock = config.get(category9, "Enable crafting for block with old lapis texture(2x2 lapis)", true).getBoolean(true);
        enablewhitesoulsand = config.get(category9, "Enable white soul sand crafting?", true).getBoolean(true);
    ...
    /* public static void dimensionconfig (File configBase)
    {
        ...
        vmdeserthillsid = config.get(miningdimensionids, "Vanity Dimension Desert Hills World ID", 253).getInt(253);
        vmextremehillsid = config.get(miningdimensionids, "Vanity Dimension Extreme Hills World ID", 252).getInt(252);
        vmforestid = config.get(miningdimensionids, "Vanity Dimension Forest World ID", 251).getInt(251);
        vmtaigaid = config.get(miningdimensionids, "Vanity Dimension Taiga World ID", 250).getInt(250);
        vmswamplandid = config.get(miningdimensionids, "Vanity Dimension Swampland World ID", 249).getInt(249);
        ...
        vmenableextremehills = config.get(miningdimensionsenable, "Enable the Extremehills mining dimension?", true).getBoolean(true);
        vmenableforest = config.get(miningdimensionsenable, "Enable the Forest mining dimension?", true).getBoolean(true);
        vmenabletaiga = config.get(miningdimensionsenable, "Enable the Taiga mining dimension?", true).getBoolean(true);
        vmenableswampland = config.get(miningdimensionsenable, "Enable the Swampland mining dimension?", true).getBoolean(true);
        vmenablehell = config.get(miningdimensionsenable, "Enable the Hell mining dimension?", true).getBoolean(true);
        vmenableshroom = config.get(miningdimensionsenable, "Enable the Shroom mining dimension?", true).getBoolean(true);
        vmenablejungle = config.get(miningdimensionsenable, "Enable the Jungle mining dimension?", true).getBoolean(true);
