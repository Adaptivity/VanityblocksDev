            ...
            try
            {
                ...
                while ((str = in.readLine()) != null)
                {
                    ...
                    if (str.contains("Version"))
                    {
                        ...
                        if (DefaultProps.remoteMajVer > DefaultProps.LOCALMAJVERSION || DefaultProps.remoteMajVer == DefaultProps.LOCALMAJVERSION
                                && DefaultProps.remoteMinVer > DefaultProps.LOCALMINVERSION || DefaultProps.remoteMajVer == DefaultProps.LOCALMAJVERSION
                                && DefaultProps.remoteMinVer == DefaultProps.LOCALMINVERSION && DefaultProps.remoteBuildVer > DefaultProps.LOCALBUILDVERSION)
                        {
                            Thread.sleep(15000);

                            if (sideToCheck.equals(Side.CLIENT))
                            {
                                FMLClientHandler.instance().getClient().thePlayer.sendChatMessage("\u00a77New \u00a73VanityBlocks \u00a77version available! v"
                                        + String.valueOf(DefaultProps.remoteMajVer) + "." + String.valueOf(DefaultProps.remoteMinVer) + "." + String.valueOf(DefaultProps.remoteBuildVer)
                                        + " http://www.minecraftforum.net/topic/1631527- or http://rly.cc/yl8ky (Jenkins)");
                                FMLClientHandler.instance().getClient().thePlayer
                                        .sendChatMessage("\u00a73VanityBlocks: \u00a77Please remember to post ideas, bugs and just general bump the thread. More attention/bumps makes me wanna continue :)");
                            }
                            else if (sideToCheck.equals(Side.SERVER))
                            {
                                VbLog.severe("New VanityBlocks version available! v" + String.valueOf(DefaultProps.remoteMajVer) + "." + String.valueOf(DefaultProps.remoteMinVer) + "."
                                        + String.valueOf(DefaultProps.remoteBuildVer) + " http://www.minecraftforum.net/topic/1631527-");
