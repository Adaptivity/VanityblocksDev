...
public class MarbleGen implements IWorldGenerator
{
    public MarbleGen(int par1)
    {
        ...
        // 100, true, false, Block.netherrack.blockID);
        marble = new WorldGenMinable(VanityBlocksRegistration.VanityDesignworldblock, 0, (Storageprops.marblevein), Blocks.stone);
        blackmarble = new WorldGenMinable(VanityBlocksRegistration.VanityDesignworldblock, 5, (Storageprops.blackmarblevein), Blocks.stone);
    }
    ...
    void generateSurface (Random random, int xChunk, int zChunk, World world)
    {
        ...
        if (Storageprops.generatemarble)
        {
            for (int q = 0; q < (Storageprops.marblerarity); q++)
            {
                ...
                yPos = (Storageprops.marbleheight) + random.nextInt(Storageprops.marblerange);
    ...
    void generateblackmarble (Random random, int xChunk, int zChunk, World world)
    {
        ...
        if (Storageprops.generateblackmarble)
        {
            ...
            {
                xPos = xChunk + random.nextInt(16);
                yPos = (Storageprops.blackmarbleheight) + random.nextInt(Storageprops.blackmarblerange);
                zPos = zChunk + random.nextInt(16);
