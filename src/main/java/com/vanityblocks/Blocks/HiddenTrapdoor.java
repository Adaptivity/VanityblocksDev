 ...
 * For use with my pack i play in mc, without the rest of his mod
 */
public class HiddenTrapdoor extends Block {
	...
	public int colorMultiplier(IBlockAccess world, int x, int y, int z) {
		...
		if (Blocks.grass.getBlockTextureFromSide(1).equals(
				getIcon(world, x, y, z, 1))) {
			int red = 0;
			int green = 0;
			int blue = 0;
			...
			for (int offsetX = -1; offsetX <= 1; ++offsetX) {
				for (int offsetZ = -1; offsetZ <= 1; ++offsetZ) {
					int color = world.getBiomeGenForCoords(x + offsetZ,
							z + offsetX).getBiomeGrassColor(x + offsetZ, y,
							z + offsetX);
			...
			return (red / 9 & 0xff) << 16 | (green / 9 & 0xff) << 8 | blue / 9
					& 0xff;
		}
	...
	public void onNeighborBlockChange(World par1World, int par2, int par3,
		...
		if (!par1World.isRemote) {
			...
			if (!(isValidSupportBlock(par1World.getBlock(j1, par3, k1)) || par1World
					.isSideSolid(j1, par3, k1,
	...
	public boolean canPlaceBlockOnSide(World world, int x, int y, int z,
		...
		if (meta == 0)
			...
			return isValidSupportBlock(world.getBlock(x, y, z))
	...
	private static boolean isValidSupportBlock(Block block) {
		if (block == null)
			...
			return block.getMaterial().isOpaque()
					&& block.renderAsNormalBlock() || block == Blocks.glowstone
