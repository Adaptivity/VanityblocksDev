package com.vanityblocks.Renders;
...
public class BlockCurtainRender implements ISimpleBlockRenderingHandler {
	...
	public void renderInventoryBlock(Block block, int metadata, int modelID,
		...
		if (modelID == curtainModelID) {
			if (metadata == 0) {
				// Bar rod east-west
				renderer.setRenderBounds(0.0F, 0.85F, 0.375F, 1.0F, 1.0F,
						0.625F);
		...
		if (modelID == curtainModelID) {
			...
			if (modelID == curtainModelID) {
				if (metadata == 1) {
					...
					renderer.renderStandardBlock(block, x, y, z);
					// 5 line
					renderer.setRenderBounds(0.5F, -1F, 0.62F, 0.435F, 1F,
							0.50F);
					renderer.renderStandardBlock(block, x, y, z);
					// 4 line
					...
					renderer.renderStandardBlock(block, x, y, z);
					// 3 line
					renderer.setRenderBounds(0.5F, -1F, 0.38F, 0.435F, 1F,
							0.25F);
					renderer.renderStandardBlock(block, x, y, z);
					// 2 line
