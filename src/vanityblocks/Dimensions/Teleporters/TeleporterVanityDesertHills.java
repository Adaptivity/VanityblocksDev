package vanityblocks.Dimensions.Teleporters;
...
import vanityblocks.Dimensions.PortalPositions.VanityPortalPositionsDesertHills;
...
public class TeleporterVanityDesertHills extends Teleporter
{
    ...
    public TeleporterVanityDesertHills(WorldServer par1WorldServer)
    {
            ...
            for (int l = -2; l <= 2; ++l)
            {
                for (int x1 = -2; x1 <= 2; ++x1)
                {
                    for (int y1 = -1; y1 < 3; ++y1)
                    {
                        ...
                        int x2 = z + x1 * b1 - l * b0;
                        boolean flag = y1 < 0;
                        this.worldServerInstance.setBlock(z1, y1, x2, flag ? Block.stone.blockID : 0);
                    }
                }
            }

            par1Entity.setLocationAndAngles((double) x, (double) y, (double) z, par1Entity.rotationYaw, 0.0F);
    ...
    public boolean placeInExistingPortal (Entity par1Entity, double par2, double par4, double par6, float par8)
    {
        ...
        else
        {
            for (k1 = l - short1; k1 <= l + short1; ++k1)
            {
                ...
                for (int l1 = i1 - short1; l1 <= i1 + short1; ++l1)
                {
                    ...
                    {
                        if (this.worldServerInstance.getBlockId(k1, i2, l1) == MiningWorldRegistrations.vanityportal.blockID)
                        {
                            while (this.worldServerInstance.getBlockId(k1, i2 - 1, l1) == MiningWorldRegistrations.vanityportal.blockID)
                            {
                                --i2;
                            }
        ...
        if (d3 >= 0.0D)
        {
            if (flag)
            {
                this.field_85191_c.add(j1, new VanityPortalPositionsDesertHills(this, i, j, k, this.worldServerInstance.getTotalWorldTime()));
            ...
            if (this.worldServerInstance.getBlockId(i - 1, j, k) == MiningWorldRegistrations.vanityportal.blockID)
            {
                j2 = 2;
            }
            ...
            if (this.worldServerInstance.getBlockId(i, j, k - 1) == MiningWorldRegistrations.vanityportal.blockID)
            {
                j2 = 3;
            }
            ...
            for (k2 = -1; k2 <= 1; ++k2)
            {
                for (i3 = 1; i3 < 3; ++i3)
                {
                    for (l2 = -1; l2 < 3; ++l2)
                    {
                        ...
                        i4 = j2 + (i3 - 1) * l5 - k2 * k5;
                        flag = l2 < 0;
                        this.worldServerInstance.setBlock(k3, j3, i4, flag ? Block.stone.blockID : 0);
                    }
                }
            }
        }
        ...
        for (k2 = 0; k2 < 4; ++k2)
        {
            for (i3 = 0; i3 < 4; ++i3)
            {
                for (l2 = -1; l2 < 4; ++l2)
                {
                    ...
                    this.worldServerInstance.setBlock(k3, j3, i4, flag ? Block.stone.blockID : MiningWorldRegistrations.vanityportal.blockID, 1, 2);
                }
            }
