...
public class BlockProperties
{
    ...
    private static void playBlockPlacementSound (TileCurtain TE, int blockID)
    {
        if (!TE.worldObj.isRemote && blockID > 0)
        {
            ...
            TE.worldObj.playSoundEffect(TE.xCoord + 0.5F, TE.yCoord + 0.5F, TE.zCoord + 0.5F, block.stepSound.getPlaceSound(), block.stepSound.getVolume() + 1.0F / 2.0F,
    ...
    public final static boolean isCover (ItemStack itemStack)
    {
        if (itemStack.getItem() instanceof ItemBlock) //&& !isSoil(itemStack) && !isPlant(itemStack)
        {
            ...
            return !block.hasTileEntity(itemStack.getItemDamage()) && (block.renderAsNormalBlock() || block instanceof BlockHalfSlab || block instanceof BlockPane || block instanceof BlockBreakable);
        }
